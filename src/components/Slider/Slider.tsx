...
import { updateSettings } from '../../services/settingsSlice'
...
function Slider({ sliderName, fromCenter, value, id }: TSliderProps) {
    const minValue = -100
    const maxValue = 100
    ...
    const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        dispatch(updateSettings({ id, settings: { [sliderName.toLowerCase()]: newValue } }))
    }
    ...
    const renderValue = (value: number, sliderName: string) => {
        switch (sliderName) {
            ...
            case 'Scale':
                return roundValue(value, minValue, maxValue, 0.1, 1.9).toFixed(1)
            case 'Blur':
